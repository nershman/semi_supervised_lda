...
from lda.ldamodel import LdaModel, LdaState
...
class LdaMulticore(LdaModel):
    ...
    def __init__(self, corpus=None, num_topics=100, id2word=None, workers=None,
                 ...
                 gamma_threshold=0.001,defined_kws={}, tfMod=None,topic_names=None):
        """
        ...
        self.topic_map = {(id, topic) for (id, topic) in enumerate(topic_names)}
        ...
        super(LdaMulticore, self).__init__(corpus=corpus, num_topics=num_topics,
            ...
            gamma_threshold=gamma_threshold,defined_kws=defined_kws, tfMod=tfMod)

    def print_doc_topics(self, bow):
        t = self[bow]
        t.sort(key=lambda x: -x[1])
        topics = self.topic_map
        for p in t:
            if p[0] in topics:
                print u'-#{} - {} : {}'.format(p[0],topics[p[0]], p[1])
            else:
                print u'-{} : {}'.format(p[0], p[1])
