...
class LdaModel(interfaces.TransformationABC):
    ...
    def __init__(self, corpus=None, num_topics=100, id2word=None,
                 ...
                 minimum_probability=0.01, defined_kws={}, tfMod=None):
        """
    ...
    def inference(self, chunk, collect_sstats=False, test=False):
        """
        ...
        if not test:
            defined_wordids = self.defined_wordids
            for i in xrange(len(chunk)):
                doc = chunk[i]
                t_count = numpy.zeros((self.num_topics,))
                nonempty = False
                for w, c in doc:
                    if w in defined_wordids:
                        # print 'not empty'
                        nonempty = True
                        topics = defined_wordids[w]
                        t_count[topics] += c
                        # for t in topics:
                            # t_count[t] += c
                ...
                if nonempty:
                    # print 'recompute'
                    #recompute topic distribution of doc base on define words
                    #norm = t_count.max() * (1.0 / self.num_topics) #numpy.linalg.norm(t_count)
                    norm = numpy.sum(t_count) / self.num_topics
                    t_count /= norm
                    #print t_count
                    gamma[i, :] = numpy.random.gamma(0.1, 0.05, (self.num_topics,))
                    gamma[i, :] += t_count
                    # print gamma[i]
    ...
    def get_document_topics(self, bow, minimum_probability=None):
        ...
        gamma, _ = self.inference([bow], test=True)
